...
import { ColumnsGiftCard } from 'columns/gift-card-transactions';
...
const GiftCard = ({ id }) => {
  ...
  const handleExport = async () => {
    setIsLoading(true);
    const data = await ReportService.exportGiftCardTransactions(dataTableRef?.current?.params, 'gift_card');
    if (data) {
      window.open(data);
    }
    setIsLoading(false);
  ...
  return (
    <Fragment>
      <DataTable
        ...
        id={() => id}
        Get={async (params) => {
          return ReportService.getGiftCardTransactions(params, 'gift_card');
        }}
        columns={ColumnsGiftCard()}
